...
"    vec4 texColor;\n"
"    if (vHeight < 0.3) {\n"
"        texColor = texture2D(texture1, vTexCoord);\n"
"    } else if (vHeight < 0.6) {\n"
"        texColor = texture2D(texture2, vTexCoord);\n"
"    } else {\n"
"        texColor = texture2D(texture3, vTexCoord);\n"
"    }\n"
...
float cameraDistance = 4.0f; // 初始视角距离
...
void init() {
    ...
    texture2 = loadTexture("Land.bmp");
    texture3 = loadTexture("Grass1.bmp");
    ...
    for (int z = -HEIGHT / 2; z < HEIGHT / 2; z += step) {
        for (int x = -WIDTH / 2; x < WIDTH / 2; x += step) {
            ...
            // 添加颜色数据
            float color = height * WIDTH / 30.0f;
            ...
            // 添加纹理坐标
            vertices.push_back(texCoords[texCoordIndex % 3][0]); // Texture coordinate x
            vertices.push_back(texCoords[texCoordIndex % 3][1]); // Texture coordinate y
            texCoordIndex++;
        }
    }
    ...
    for (int y = 0; y < HEIGHT / step - 1; ++y) {
        for (int x = 0; x < WIDTH / step - 1; ++x) {
            ...
            indices.push_back(start + 1);
            indices.push_back(start + 1 + WIDTH / step);
            indices.push_back(start + WIDTH / step);
    ...
    std::cout << "posAttrib: " << posAttrib << std::endl; // 调试输出
    GL_CHECK(glVertexAttribPointer(posAttrib, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(GLfloat), (GLvoid*)0));
    GL_CHECK(glEnableVertexAttribArray(posAttrib));

    GLint colorAttrib = glGetAttribLocation(shaderProgram, "aColor");
    std::cout << "colorAttrib: " << colorAttrib << std::endl; // 调试输出
    GL_CHECK(glVertexAttribPointer(colorAttrib, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(GLfloat), (GLvoid*)(3 * sizeof(GLfloat))));
    GL_CHECK(glEnableVertexAttribArray(colorAttrib));

    GLint texCoordAttrib = glGetAttribLocation(shaderProgram, "aTexCoord");
    std::cout << "texCoordAttrib: " << texCoordAttrib << std::endl; // 调试输出
    GL_CHECK(glVertexAttribPointer(texCoordAttrib, 2, GL_FLOAT, GL_FALSE, 8 * sizeof(GLfloat), (GLvoid*)(6 * sizeof(GLfloat))));
